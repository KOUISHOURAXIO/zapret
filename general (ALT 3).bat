...
start "zapret: general (ALT 3)" /min "%BIN%winws.exe" --wf-tcp=80,443 --wf-udp=443,50000-50100 ^
...
--filter-tcp=443 --hostlist-exclude="%LIST%list-exclude.txt" --ipset="%LIST%list-ipset.txt" --hostlist="%LIST%list.txt" --hostlist="%LIST%list-additional.txt" --hostlist-auto-fail-time=3 --hostlist-auto-fail-threshold=1 --dpi-desync=split --dpi-desync-split-pos=1 --dpi-desync-autottl --dpi-desync-fooling=badseq --dpi-desync-repeats=8
